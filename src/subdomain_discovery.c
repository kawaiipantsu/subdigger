...
int discover_subdomains(subdigger_ctx_t *ctx) {
    ...
    if (method_enabled(config, "cert")) {
        ...
        if (cert_results) {
            for (size_t i = 0; i < cert_count; i++) {
                task_queue_push(ctx->task_queue, cert_results[i]);
                total_candidates++;
            }
    ...
    if (method_enabled(config, "wordlist")) {
    ...
    if (method_enabled(config, "dns")) {
        ...
        if (axfr_results) {
            for (size_t i = 0; i < axfr_count; i++) {
                task_queue_push(ctx->task_queue, axfr_results[i]);
                total_candidates++;
            }
    ...
    if (method_enabled(config, "api")) {
        if (config->api_key_shodan) {
            ...
            if (shodan_results) {
                for (size_t i = 0; i < shodan_count; i++) {
                    task_queue_push(ctx->task_queue, shodan_results[i]);
                    total_candidates++;
                }
        ...
        if (config->api_key_virustotal) {
            ...
            if (vt_results) {
                for (size_t i = 0; i < vt_count; i++) {
                    task_queue_push(ctx->task_queue, vt_results[i]);
                    total_candidates++;
                }
    ...
    if (method_enabled(config, "bruteforce")) {
